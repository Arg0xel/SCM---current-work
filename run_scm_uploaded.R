...
# 6. Check minimum coverage for donor pool (outcome AND predictors)
donor_coverage <- wdi_data %>%
  ...
  filter(
    ...
    predictor_1_coverage >= config$min_pre_coverage,
...
cat(sprintf("Final donor pool after coverage filter (outcome + predictors): %d countries\n",
            length(donor_pool)))
