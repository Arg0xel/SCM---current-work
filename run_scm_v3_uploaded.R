...
# This addresses the convex hull requirement for credible counterfactuals
if (nrow(donor_coverage) > 50) {
  cat(sprintf("Large donor pool (%d countries) - this is GOOD for methodological soundness\n", nrow(donor_coverage)))
  cat("Keeping all eligible donors to satisfy convex hull requirement\n")
}
...
# Fit Synth with timeout protection
cat("Fitting synthetic control (this may take a few minutes)...\n")
...
tryCatch({
  ...
  if (elapsed_time > max_duration) {
    warning("Synth optimization took longer than 5 minutes. Results may be unstable.")
  }
  ...
  tryCatch({
    ...
    stop(sprintf(
      paste("synth optimization failed after multiple attempts:\n",
            "Primary error: %s\n",
            "Secondary error: %s\n",
            "\nPossible solutions:\n",
...
if (n_nonzero_weights < 5) {
  warning(sprintf(paste(
    ...
    "This suggests:\n",
    "  1. Predictor data may be very sparse or misaligned\n",
